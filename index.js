...
function checkValueAndType(mySet, value) {
  ...
  // Якщо значення знайдено, повертаємо рядок .
...
function setToArray(mySet) {
  ...
  const fiteredArray = arrayFromSet.filter(
    (element) => typeof element === "string"
  );
  const sortedArray = fiteredArray.sort();
  return sortedArray;
...
function getIntersection(arr1, arr2) {
  ...
  for (let element of set1) {
    if (set2.has(element)) intersectionSet.add(element);
  }
